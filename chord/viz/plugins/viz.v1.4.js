! function() {
    ...
    viz.biPartite = function() {
        var key_scale, value_scale, keyPrimary, keySecondary, value, width, height, orient, barSize, min, pad, data, fill, sel, edgeOpacity, duration, sortPrimary, sortSecondary, edgeMode, refresh = true,
            bars
    ...
    viz.chord = function() {
        var data, fill, duration, chordOpacity, innerRadius, outerRadius, source, target, value, padAngle, labelPadding, sort, startAngle, chords, groups, label, min = 0,
    ...
    viz.legend = function() {
        var sel, data, rows, cols, width, paddingInner, paddingLabel, size, fill, rowScale, colScale, draw, onMouseOut, onMouseOver
        ...
        function legend(_sel) {
            ...
            var cellHeight = rowScale.bandwidth(),
    ...
    viz.pc = function() {
        var g, data, dimensions, dimensionLabelPadding, dimensionScale, valueScale, width, height, outerPadding, dimensionAxes, brushSize;
        var dragging = {},
            background, foreground, brushSelections = {},
            path;
    ...
    viz.area = function() {
        var sel, data, width, height, key, value0, value1, keyScale, valueScale, defined, curve, duration, ease, orient, orients = { 'bottom': 1, 'top': 2, 'left': 3, 'right': 4 }
    ...
    viz.line = function() {
        var sel, data, width, height, key, value, keyScale, valueScale, defined, curve, duration, ease, orient, orients = { 'bottom': 1, 'top': 2, 'left': 3, 'right': 4 };
    ...
    viz.point = function() {
        var sel, data, width, height, key, value, keyScale, valueScale, defined, curve, duration, ease, drawPoints, tree, points, refresh = true,
        ...
        function updateLocals() {
            ...
            tree = d3.quadtree()
                ...
                .addAll(points)
                .extent([
                    [keyScale.range()[0], valueScale.range()[0]],
                    [keyScale.range()[1] + 1, valueScale.range()[1] + 1]
                ]);
            refresh = false;
        }
    ...
    viz.bar = function() {
        var sel, data, width, height, key, value0, value1, keyScale, valueScale, defined, curve, duration, ease, align, paddingInner, paddingOuter, orient, orients = { 'bottom': 1, 'top': 2, 'left': 3, 'right': 4 };
    ...
    viz.pie3d = function() {
        var g, data, innerRadius, outerRadius, value, fill, startAngle, endAngle, eccentricity, height, label
        ...
        function pie3d(_g) {
            ...
            function pieTop(d) {
                ...
                return ["M", sx, sy, "A", rx, ry, "0", (d.endAngle - d.startAngle > pi ? 1 : 0), "1", ex, ey, "L", ir * ex, ir * ey,
                    "A", ir * rx, ir * ry, "0", (d.endAngle - d.startAngle > pi ? 1 : 0), "0", ir * sx, ir * sy, "z"
                ].join(" ");
            }
    ...
    viz.pie = function() {
        var g, data, innerRadius, outerRadius, cornerRadius, startAngle, endAngle, padAngle, value, fill, sort, label, ease, duration, folded = false
    ...
    viz.form.checkList = function() {
        var data, sel, buttonLabel, optionLabel, onClick, dataValue, defaultCheck, selectedIndices, onHidden, allNoneButton;
    ...
    function boxWhisker(a) {
        var sa = a.filter(function(d) { return !isNaN(d); }).sort(d3.ascending),
            l = sa.length;
    ...
    function viz_arc(x) {
        ...
        return ["M", ss, "A", x[0], x[0], "0", (x[3] - x[2] > pi ? 1 : 0), "1", se, "L", ee,
            "A", x[1], x[1], "0", (x[3] - x[2] > pi ? 1 : 0), "0", es, "z"
        ].join(" ");
    }
